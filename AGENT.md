...
41. **Periodic survey for proposal quality feedback:** After every Nth archive action (default 5, configurable via `SURVEY_INTERVAL` exported from the swipe route), the swipe endpoint returns `showSurvey: true` in the response. The `SwipeQueue` component then displays a `SurveyModal` (`src/components/survey-modal.tsx`) — a modal dialog asking "What's the most common issue you've seen in recent proposals?" with seven multi-select failure mode options (scientifically_nonsensical, scientifically_uninteresting, lack_of_synergy, experiment_too_complex, too_generic, already_pursuing_similar, other). Selecting "Other" reveals an optional free-text textarea (max 1000 chars). Users can skip without submitting. `POST /api/survey` stores responses as `SurveyResponse` records (failureModes string array + optional freeText). The endpoint validates failure mode values against `VALID_FAILURE_MODES`. The swipe endpoint determines survey timing by counting the user's total archive swipes (including the current one) and checking divisibility by SURVEY_INTERVAL. Tests: `survey/__tests__/route.test.ts` (12 tests), `survey-modal.test.tsx` (13 tests), plus 4 new tests in `swipe/__tests__/route.test.ts`. Focused verification: `source ~/.nvm/nvm.sh && npx jest src/app/api/survey/__tests__/route.test.ts src/components/__tests__/survey-modal.test.tsx src/app/api/proposals/\\[id\\]/swipe/__tests__/route.test.ts`.

42. **AWS SES email service:** `src/lib/ses.ts` provides the SES client singleton (same global pattern as Prisma/Anthropic). `src/services/email-service.ts` is the email service providing template rendering and sending for all 4 notification types: `match_notification`, `new_proposals_digest`, `profile_refresh_candidate`, `unclaimed_profile_recruitment`. Each template has a dedicated render function returning `{ subject, html }`. HTML emails use inline styles with a shared layout wrapper (CoPI branding header, body content, footer with notification preferences link). Template data is HTML-escaped via `escapeHtml()` to prevent XSS. When AWS credentials are not configured (`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` missing), `sendEmail()` logs to console instead of sending (dev mode). The `send_email` worker handler in `src/worker/handlers.ts` calls `sendTemplatedEmail(sesClient, templateId, to, data)` which renders the template and sends via SES. Errors propagate for queue retry with exponential backoff. The SES client is injected for testability. `@aws-sdk/client-ses` is a production dependency. Focused verification: `source ~/.nvm/nvm.sh && npx jest src/services/__tests__/email-service.test.ts src/worker/__tests__/handlers.test.ts`.

43. **Match notification email triggering:** `src/services/match-notifications.ts` provides `sendMatchNotificationEmails(prisma, matchId, proposalId)` which sends immediate email notifications to both users when a mutual match is created. For each user: checks `emailNotificationsEnabled` (master switch) AND `notifyMatches` (per-type toggle), skips placeholder ORCID emails (`@orcid.placeholder`), enqueues a `send_email` job with `match_notification` template containing the user-perspective one-line summary (`oneLineSummaryA` or `oneLineSummaryB`), and updates the Match record's `notificationSentA`/`notificationSentB` flags. Contact email inclusion respects the OTHER user's `emailVisibility` setting (`public_profile` or `mutual_matches` → include, `never` → omit). Integrated into both the swipe endpoint (`POST /api/proposals/[id]/swipe`) and unarchive endpoint (`POST /api/proposals/[id]/unarchive`) as fire-and-forget calls with `.catch()` error logging — notification failures never block the swipe response. Tests: `match-notifications.test.ts` (12 tests), plus 3 new tests in `swipe/__tests__/route.test.ts` and 2 new tests in `unarchive/__tests__/route.test.ts`. Focused verification: `source ~/.nvm/nvm.sh && npx jest src/services/__tests__/match-notifications.test.ts src/app/api/proposals/\\[id\\]/swipe/__tests__/route.test.ts src/app/api/proposals/\\[id\\]/unarchive/__tests__/route.test.ts`.

44. **Weekly proposals digest email:** `src/services/proposals-digest.ts` provides `runWeeklyDigest(prisma, options?)` which sends batched digest emails to all eligible users about their new, unswiped, visible collaboration proposals. The User model has a `lastDigestSentAt` field (added via migration) to enforce the "at most one email per week" frequency cap. The service queries all users with `emailNotificationsEnabled=true` AND `notifyNewProposals=true`, excluding placeholder ORCID emails and users who received a digest within `DIGEST_INTERVAL_MS` (7 days, exported). For each eligible user: finds proposals created after `lastDigestSentAt` (or all time if null) where the user's visibility is `visible` and they haven't swiped yet, selects the highest-confidence most-recent proposal for the preview, enqueues a `send_email` job with `new_proposals_digest` template (template already implemented in email-service.ts), and updates `lastDigestSentAt`. Returns `WeeklyDigestResult` with `emailsSent`, `usersSkipped`, and per-user `errors`. Errors on one user don't block processing of others. The `minIntervalMs` option allows overriding the interval for testing. The scheduler that calls `runWeeklyDigest` on a configurable day (default: Monday per spec) is pending infrastructure wiring. Tests: `proposals-digest.test.ts` (16 tests). Focused verification: `source ~/.nvm/nvm.sh && npx jest src/services/__tests__/proposals-digest.test.ts`.
